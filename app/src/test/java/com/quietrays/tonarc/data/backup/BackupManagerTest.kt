...
import com.quietrays.tonarc.data.backup.model.BackupModuleInfo
...
import com.quietrays.tonarc.data.backup.model.ModuleRestoreDetail
import com.quietrays.tonarc.data.backup.model.RestorePlan
import com.quietrays.tonarc.data.backup.model.ValidationError
...
class BackupManagerTest {
    ...
    fun setUp() {
        coEvery { backupReader.detectFormat(any()) } returns Result.success(com.quietrays.tonarc.data.backup.format.BackupFormatDetector.Format.PXPL_V3_ZIP)
    }
        ...
        every { validationPipeline.validateFile(backupUri) } returns BackupValidationResult.Invalid(
            listOf(
                ValidationError(
                    ...
                    severity = com.quietrays.tonarc.data.backup.model.Severity.WARNING
                )
            )
        )
