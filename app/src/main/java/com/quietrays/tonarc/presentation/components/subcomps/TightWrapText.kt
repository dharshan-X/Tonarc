...
/**
 * Text that reports only the width its glyphs actually occupy, instead of the width the layout
 * reserved for it. Useful inside buttons whose labels wrap to two lines once translated: a normal
 * [androidx.compose.material3.Text] would keep the full line width and push the button wider than
 * the content needs.
 */
@Composable
fun TightWrapText(
    ...
    Layout(
        ...
        layout(tightWidth, result.size.height) {
            // Nothing to place: the text is drawn behind the layout node.
        }
    }
}
