...
class WidgetUpdateReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != "com.quietrays.tonarc.ACTION_WIDGET_UPDATE_PLAYBACK_STATE") return
        ...
        scope.launch {
            try {
                ...
                val glanceIds = glanceAppWidgetManager.getGlanceIds(TonarcGlanceWidget::class.java)
                glanceIds.forEach { glanceId ->
                    TonarcGlanceWidget().update(context, glanceId)
                }
