...
class ImageCacheManager @Inject constructor(
    ...
    fun invalidateCoverArtCaches(vararg uriStrings: String?) {
        ...
        uriStrings.mapNotNull { it?.takeIf(String::isNotBlank) }.forEach { baseUri ->
            if (com.quietrays.tonarc.utils.LocalArtworkUri.isLocalArtworkUri(baseUri)) {
                com.quietrays.tonarc.utils.LocalArtworkUri.parseSongId(baseUri)?.let { songId ->
                    com.quietrays.tonarc.utils.AlbumArtUtils.clearCacheForSong(context, songId)
                }
            }
