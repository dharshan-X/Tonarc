...
import com.quietrays.tonarc.MainActivity
import com.quietrays.tonarc.R
import com.quietrays.tonarc.data.diagnostics.AdvancedPerformanceDiagnostics
import com.quietrays.tonarc.data.model.PlayerInfo
import com.quietrays.tonarc.data.model.PlaybackQueueItemSnapshot
import com.quietrays.tonarc.data.model.PlaybackQueueSnapshot
import com.quietrays.tonarc.data.preferences.EqualizerPreferencesRepository
...
import com.quietrays.tonarc.data.service.player.TransitionController
import com.quietrays.tonarc.ui.glancewidget.ControlWidget4x2
import com.quietrays.tonarc.ui.glancewidget.TonarcGlanceWidget
import com.quietrays.tonarc.ui.glancewidget.PlayerActions
import com.quietrays.tonarc.ui.glancewidget.PlayerInfoStateDefinition
import com.quietrays.tonarc.utils.AlbumArtUtils
...
import com.quietrays.tonarc.data.equalizer.EqualizerManager
import com.quietrays.tonarc.data.equalizer.ExternalAudioEffectSession
import com.quietrays.tonarc.data.model.WidgetThemeColors
...
import com.quietrays.tonarc.presentation.viewmodel.ColorSchemeProcessor
import androidx.compose.ui.graphics.toArgb
import com.quietrays.tonarc.ui.glancewidget.BarWidget4x1
import com.quietrays.tonarc.ui.glancewidget.GridWidget2x2
...
import com.quietrays.tonarc.utils.ArtworkTransportSanitizer
...
import com.quietrays.tonarc.presentation.viewmodel.ListeningStatsTracker
...
class MusicService : MediaSessionService() {
    ...
    @Inject
    lateinit var replayGainManager: com.quietrays.tonarc.data.media.ReplayGainManager
    @Inject
    lateinit var navidromeRepository: NavidromeRepository
    @Inject
    lateinit var listeningStatsTracker: ListeningStatsTracker
    @Inject
    lateinit var youTubeRepository: com.quietrays.tonarc.data.youtube.YouTubeRepository
    @Inject
    ...
    companion object {
        ...
        const val ACTION_SLEEP_TIMER_EXPIRED = "com.quietrays.tonarc.ACTION_SLEEP_TIMER_EXPIRED"
        const val EXTRA_FORCE_FOREGROUND_ON_START =
            "com.quietrays.tonarc.extra.FORCE_FOREGROUND_ON_START"
        ...
        private const val APP_PACKAGE_PREFIX = "com.quietrays.tonarc"
    ...
    private fun Player.unwrapMappingPlayer(): Player {
        return (this as? com.quietrays.tonarc.data.service.player.MappingPlayer)?.innerPlayer ?: this
    }

    private fun wrapSessionPlayer(player: Player): Player {
        return com.quietrays.tonarc.data.service.player.MappingPlayer(
            innerPlayer = player,
            context = this
        )
    }
    ...
    private fun startTemporaryForegroundForCommand() {
        val notification = NotificationCompat.Builder(
            this,
            TonarcApplication.NOTIFICATION_CHANNEL_ID
        )
    ...
    private fun checkAndTriggerInfiniteAutoplay(currentMediaItem: MediaItem?) {
        ...
        if (currentIndex != androidx.media3.common.C.INDEX_UNSET && currentIndex >= totalCount - 2) {
            ...
            serviceScope.launch {
                ...
                try {
                    val currentSong = musicRepository.getSong(mediaId).firstOrNull() ?: com.quietrays.tonarc.data.model.Song(
                        id = mediaId,
    ...
    private fun getOpenAppPendingIntent(): PendingIntent {
        val intent = Intent(this, MainActivity::class.java).apply {
            ...
            action = "com.quietrays.tonarc.action.OPEN_PLAYER"
    ...
    private suspend fun buildPlayerInfo(): PlayerInfo {
        ...
        val queueItems = mutableListOf<com.quietrays.tonarc.data.model.QueueItem>()
        if (!snapshotTimeline.isEmpty) {
            ...
            for (i in startIndex until endIndex) {
                ...
                if (songId != null) {
                    ...
                    queueItems.add(
                        com.quietrays.tonarc.data.model.QueueItem(
                            id = songId,
    ...
    public suspend fun loadArtworkBytesForWidget(uri: Uri): ByteArray? {
        ...
        val scheme = uri.scheme?.lowercase()
        val isLocalArtworkUri = com.quietrays.tonarc.utils.LocalArtworkUri.isLocalArtworkUri(uriString)
        return when {
    ...
    private suspend fun updateGlanceWidgets(playerInfo: PlayerInfo) = withContext(Dispatchers.IO) {
        try {
            ...
            val glanceIds = glanceManager.getGlanceIds(TonarcGlanceWidget::class.java)
            glanceIds.forEach { id ->
                updateAppWidgetState(applicationContext, PlayerInfoStateDefinition, id) { widgetPlayerInfo }
                TonarcGlanceWidget().update(applicationContext, id)
            }
