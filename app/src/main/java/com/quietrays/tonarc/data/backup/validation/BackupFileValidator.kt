package com.quietrays.tonarc.data.backup.validation
...
import com.quietrays.tonarc.data.backup.model.Severity
...
class BackupFileValidator @Inject constructor(
    ...
    fun validate(uri: Uri): BackupValidationResult {
        ...
        if (format == BackupFormatDetector.Format.UNKNOWN) {
            errors.add(ValidationError("FORMAT_UNKNOWN", "File is not a recognized Tonarc backup format."))
            return BackupValidationResult.Invalid(errors)
        }
